...
export const useNotesStore = create<NotesStore>((set, get) => ({
  ...
  loadNotes: async () => {
    try {
      ...
      console.error('[NotesStore] Failed to load notes from database:', error)
    }
  },
  ...
  addNote: async (content: string) => {
    ...
    if (!user) {
      console.error('[NotesStore] Cannot add a note without a logged-in user.')
      return
    }
    try {
      const noteToAdd = {
      ...
      console.error('[NotesStore] Failed to add note to database:', error)
    }
  },
